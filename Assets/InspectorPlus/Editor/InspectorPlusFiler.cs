...
public class InspectorPlusFiler {
	...
	string BuildVar(InspectorPlusVar v) {
        ...
        vars.Add(new InspectorPlusVar(" + BuildArgs(v.limitType, v.min, v.max, v.progressBar, v.iMin, v.iMax, v.active,
			       ...
			       v.space, v.labelEnabled, v.label, v.labelBold, v.labelItalic, v.labelAlign, v.buttonEnabled, v.buttonText,
	...
	void CheckExists() {
		...
		foreach (InspectorPlusVar v in tracker.GetVars()) {
			...
			hasInspector = true;
			if (v.type == typeof(float).Name) {
				hasFloat = true;
			}
			else if (v.type == typeof(int).Name) {
				hasInt = true;
			}
			if (hasFloat || hasInt) {
				hasProgressBar = true;
			...
			if (v.type == typeof(Vector2).Name) {
				hasVector2 = true;
			}
			else if (v.type == typeof(Quaternion).Name) {
				hasQuaternion = true;
			}
			else if (v.type == typeof(bool).Name) {
				hasBool = true;
			}

			if (v.vectorDrawType != InspectorPlusVar.VectorDrawType.None || v.QuaternionHandle) {
				hasScene = true;
			}
			if (v.vectorDrawType != InspectorPlusVar.VectorDrawType.None) {
				hasVectorScene = true;
			}
			if (v.QuaternionHandle) {
				hasQuaternionScene = true;
			}
			if (v.hasTooltip) {
				hasTooltip = true;
			}
			...
			if (v.textFieldDefault != "" || v.textArea == true) {
				hasText = true;
			}
		}
	}
	...
	void Write() {
		...
		if (hasInspector || hasScene) {
			...
			if (hasInspector) {
				writeString += s.GetOnGUI(hasFloat, hasInt, hasProgressBar, hasArray, hasVector2, hasQuaternion, hasBool,
					hasTooltip, hasSpace, hasTexture, hasText);
			}
			if (hasInspector) {
